...
@app.cell
def _(mo, pystac):
    catalog = pystac.Catalog.from_file("catalog/catalog.json")
...
def _(http, mo, partial, threading):
    ...
    url = "https://radiantearth.github.io/stac-browser/#/external/http://localhost:8000/catalog/catalog.json"
    mo.md(f"## STAC Browser\nServing `catalog/` on localhost:8000 → [open in STAC Browser]({url})")
    return
